...
# Load the YOLO model
model = YOLO("./models/battery_s640_1023.pt")
...
while cap.isOpened():
    ...
    if len(boxes) > 0:
        for box in boxes:
            # 检查检测框是否完全在矩形内
            if box[0] > top_left[0] and box[2] < bottom_right[0] and box[1] > top_left[1] and box[3] < bottom_right[1]:
